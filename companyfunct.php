...
//        if(!mysqli_query($conn, "INSERT INTO company (companyname, company_type, date_opened, company_desc) VALUES ('$name', '$type', DATE('$date'), '$desc')")){
...
//	    $id = $function['idcompany'];
    ...
    for($i = 0; $i<count($d);$i++){
//      if(""+mysqli_num_rows(mysqli_query($conn, "SELECT idcompany FROM company_dept WHERE idcompany = '$compID' AND department = '$d[$i]'")) == "0" && !($d[$i] == null || $d[$i] == ""))
        mysqli_query($conn, "INSERT INTO company_dept (idcompany, department) VALUES ('$compID', '$d[$i]')");
//    }
  }
}
